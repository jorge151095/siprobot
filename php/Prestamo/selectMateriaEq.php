...
$sql ="SELECT `alumno`.`id` AS alumnoId, `alumno`.`nombre` AS alumnoNombre, `alumno`.`apellido1` AS apellido1, `alumno`.`apellido2` AS apellido2, `materia`.`nombre` AS materiaNombre, `materia`.`id` AS materiaId FROM `materia_alumno` INNER JOIN `alumno` ON `alumno`.`id` = `materia_alumno`.`alumno_id` INNER JOIN `materia` ON `materia`.`id` = `materia_alumno`.`materia_id` WHERE `materia`.`id` = ".$id; 
    try {
        $response = array();
